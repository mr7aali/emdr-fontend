...
const config = {
  name: 'Social Phobia',
  items: [
    { text: 'I have feared being watched, weighed up, or found wanting.', reverse: false },
    { text: 'I have worried about doing something that would make me look foolish.', reverse: false },
    { text: 'I have skipped or pulled out of social situations I would otherwise have joined.', reverse: false },
    {
      text: 'My body has betrayed my anxiety in front of others (flushing, shaking, sweating, voice).',
      reverse: false,
    },
    { text: 'I have over-prepared what to say or planned out interactions in advance.', reverse: false },
    { text: 'I have felt at ease taking up space in front of others.', reverse: true },
    {
      text: 'After social situations I have picked over what I said and how I came across.',
      reverse: false,
    },
    {
      text: 'I have stayed quiet in groups, meetings, or classes when I had something to add.',
      reverse: false,
    },
    {
      text: 'I have found everyday acts difficult when others might see me doing them (eating, writing, signing my name).',
      reverse: false,
    },
    { text: 'Social anxiety has shrunk the life I want to live.', reverse: false },
  ],
  bands: [
    {
      max: 9,
      label: 'Minimal',
      description:
        'Few signs of social anxiety over the past week. The level of social discomfort described is within the range of everyday experience.',
    },
    {
      max: 17,
      label: 'Mild',
      description:
        'Some social anxiety present. You may notice it in particular situations but it is unlikely to be substantially disrupting daily life. Worth tracking as you work through the programme.',
    },
    {
      max: 25,
      label: 'Moderate',
      description:
        'A meaningful pattern of social anxiety over the past week. EMDR can help address the experiences that shape social fears. Track this score over time to notice changes.',
    },
    {
      max: 32,
      label: 'Marked',
      description:
        'Substantial social anxiety with likely impact on work, relationships, and wellbeing. Continue with the programme, and consider whether additional support might be useful.',
    },
    {
      max: 40,
      label: 'Severe',
      description:
        'A high level of social anxiety with significant impact across multiple areas of life. At this level, additional support is recommended alongside the programme - please consider speaking to a qualified mental health professional.',
    },
  ],
  options: [
    { value: 0, label: 'Never' },
...
export default function Pcl5PtsdChecklistSession4Page() {
  ...
  return (
    <div
      className={`${fraunces.variable} ${instrumentSans.variable} rounded-2xl bg-[#F5F1EA]/50 font-sans text-[#1A1814] min-h-screen antialiased selection:bg-[#5C5E8B]/20`}
    >
      <div className="mx-auto max-w-[720px] px-5 py-10 pb-20 md:px-8 md:py-16 md:pb-32">
        <header className="mb-12 border-b border-[#DDD5C5] pb-8">
          <div className="mb-6 flex items-center gap-3 text-[11px] font-medium uppercase tracking-[0.18em] text-[#8A8278]">
            <span className="h-px w-6 bg-[#5C5E8B]"></span>
            InKind EMDR | Symptom Tracker | 04
          </div>
          ...
          <p className="font-fraunces max-w-[560px] text-xl font-light leading-relaxed text-[#4A4540] opsz-36">
            A brief check-in on anxiety in social and performance situations over the past week.
          </p>
        </header>

        <div className="my-10 border-l-2 border-[#5C5E8B] bg-[#FBF8F2] p-6 text-sm leading-relaxed text-[#4A4540] md:p-8">
          ...
          <div className="mt-6 grid grid-cols-2 gap-px overflow-hidden border border-[#DDD5C5] bg-[#DDD5C5] md:grid-cols-5">
            {config.options.map((option) => (
              <div key={option.value} className="bg-[#FBF8F2] p-3 text-center">
                <div className="font-fraunces mb-1 text-lg font-medium text-[#5C5E8B]">{option.value}</div>
        ...
        <form onSubmit={calculateScore}>
          <div>
            {config.items.map((item, index) => (
              <div key={index} className="border-t border-[#EAE3D3] py-8 last:border-b last:border-[#EAE3D3]">
                <span className="font-fraunces mb-3 block text-[13px] tracking-wide text-[#5C5E8B]">
                  ...
                  {config.options.map((option) => (
                    <div key={option.value} className="relative">
                      ...
                      <label
                        ...
                        className="block cursor-pointer select-none border border-[#DDD5C5] bg-[#FBF8F2] px-4 py-3 text-center transition-all duration-150 hover:border-[#5C5E8B] hover:bg-[#EAEAF1] peer-checked:border-[#5C5E8B] peer-checked:bg-[#5C5E8B] peer-checked:text-[#F5F1EA] md:px-2 md:py-3.5"
                      >
                        <span className="font-fraunces mb-0.5 block text-lg font-medium">{option.value}</span>
                        <span className="text-[11px] uppercase tracking-widest text-[#8A8278] peer-checked:text-[#CFCFE0] md:text-[9px]">
                          {option.label}
                        </span>
                      </label>
                    </div>
          ...
          <div className="mt-16 text-center">
            ...
            <button
              type="submit"
              disabled={!isComplete}
              className="bg-[#1A1814] px-12 py-4 text-[14px] font-medium uppercase tracking-[0.06em] text-[#F5F1EA] transition-all duration-200 hover:-translate-y-px hover:bg-[#5C5E8B] disabled:cursor-not-allowed disabled:opacity-40"
            >
              Calculate score
            </button>
          </div>
                ...
                {results.itemScores.map((score, index) => (
                  <div
                    ...
                    className={`relative flex aspect-square items-center justify-center border border-[#DDD5C5] font-fraunces text-lg tabular-nums
                      ...
                      ${score.scored === 1 ? 'bg-[#EAEAF1] text-[#1A1814]' : ''}
                      ${score.scored === 2 ? 'bg-[#CFCFE0] text-[#1A1814]' : ''}
                      ${score.scored === 3 ? 'bg-[#8E90B4] text-white' : ''}
                      ${score.scored === 4 ? 'bg-[#5C5E8B] text-white' : ''}
                    `}
