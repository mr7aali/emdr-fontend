...
export default function Pcl5PtsdChecklistSession6Page() {
  ...
  const calculateScore = async (e) => {
    ...
    try {
      ...
      console.error('Failed to submit specific phobia tracker:', error);
      ...
      <div className="mx-auto max-w-[720px] px-5 py-10 pb-20 md:px-8 md:py-16 md:pb-32">
        ...
        {showResults && results && (
          ...
          >
            ...
            {submitError ? (
              <div className="mb-6 border-l-2 border-[#7D5A3D] bg-[#F1EEEC] p-4 text-sm leading-relaxed text-[#1A1814]">
                {submitError}
              </div>
            ) : null}
            ...
            <div className="mb-2 grid grid-cols-5 gap-0.5">
              {[0, 1, 2, 3, 4].map((index) => (
                <div
                  key={index}
                  className={`h-1.5 transition-colors duration-500 ${index <= results.bandIndex ? 'bg-[#7D5A3D]' : 'bg-[#DDD5C5]'
                    }`}
                />
              ))}
            </div>
            ...
            <div className="mt-12 flex flex-wrap gap-4">
              ...
              <Link
                ...
                className="text-[12px] bg-[#1A1814] text-[#F5F1EA] px-6 py-3 transition-all hover:bg-[#7D5A3D] flex items-center gap-2"
              >
                View Progress Chart
                <svg
