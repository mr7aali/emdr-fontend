...
export default function Pcl5PtsdChecklistSession3Page() {
  ...
  const calculateScore = async (e) => {
    ...
    try {
      ...
      console.error('Failed to submit anger tracker:', error);
          ...
          >
            ...
            <div className="mb-2 grid grid-cols-5 gap-0.5">
              {[0, 1, 2, 3, 4].map((index) => (
                <div
                  key={index}
                  className={`h-1.5 transition-colors duration-500 ${index <= results.bandIndex ? 'bg-[#A8553D]' : 'bg-[#DDD5C5]'
                    }`}
                />
              ))}
            </div>
            ...
            <div className="mt-12 flex flex-wrap gap-4">
              ...
              <Link
                ...
                className="text-[12px] bg-[#1A1814] text-[#F5F1EA] px-6 py-3 transition-all hover:bg-[#A8553D] flex items-center gap-2"
              >
                View Progress Chart
                <svg
