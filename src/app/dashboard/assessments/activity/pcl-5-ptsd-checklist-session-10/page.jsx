...
const config = {
  name: 'Self-Esteem',
  items: [
    { text: 'I have felt that I have value, just as I am.', reverse: false },
    { text: 'I have liked the person I see myself to be.', reverse: false },
    { text: 'I have trusted myself to handle what comes up.', reverse: false },
    { text: 'I have felt that good things in my life are mine to have.', reverse: false },
    { text: 'The voice in my head has been more friend than critic.', reverse: false },
    { text: 'I have felt that I do not measure up, or that something is wrong with me.', reverse: true },
    { text: 'I have felt at home in myself.', reverse: false },
    {
      text: 'When something kind has been said about me, I have been able to let it land.',
      reverse: false,
    },
    { text: 'What I think and feel has felt worth voicing.', reverse: false },
    { text: "My own needs have counted as real, alongside everyone else's.", reverse: false },
  ],
  bands: [
    {
      max: 9,
      label: 'Very low',
      description:
        'Few signs of healthy self-esteem in the past week. Self-esteem at this level is often associated with substantial distress, and is a useful focus as you work through the programme.',
    },
    {
      max: 17,
      label: 'Low',
      description:
        'Self-esteem has been low across most of the past week. This pattern often shapes how we treat ourselves and what we feel we deserve.',
    },
    {
      max: 25,
      label: 'Mixed',
      description:
        'A mix of healthy and unhealthy self-relating across the past week. Looking at which items scored higher and lower can be a useful map for what to build on.',
    },
    {
      max: 32,
      label: 'Healthy',
      description:
        'A largely healthy relationship with yourself across the past week. There may still be areas to strengthen - the item-by-item view shows where.',
    },
    {
      max: 40,
      label: 'Very healthy',
      description:
        'A consistently healthy relationship with yourself across the past week. Worth noticing what supports this so you can keep returning to it.',
    },
  ],
  options: [
    { value: 0, label: 'Never' },
...
export default function Pcl5PtsdChecklistSession10Page() {
  ...
  const calculateScore = async (e) => {
    ...
    try {
      ...
      console.error('Failed to submit self-esteem tracker:', error);
  ...
  return (
    <div
      className={`${fraunces.variable} ${instrumentSans.variable} rounded-2xl bg-[#F5F1EA]/50 font-sans text-[#1A1814] min-h-screen antialiased selection:bg-[#A38442]/20`}
    >
      <div className="mx-auto max-w-[720px] px-5 py-10 pb-20 md:px-8 md:py-16 md:pb-32">
        <header className="mb-12 border-b border-[#DDD5C5] pb-8">
          <div className="mb-6 flex items-center gap-3 text-[11px] font-medium uppercase tracking-[0.18em] text-[#8A8278]">
            <span className="h-px w-6 bg-[#A38442]"></span>
            InKind EMDR | Symptom Tracker | 10
          </div>
          ...
          <p className="font-fraunces max-w-[560px] text-xl font-light leading-relaxed text-[#4A4540] opsz-36">
            A brief check-in on how you have related to yourself across the past week.
          </p>
        </header>

        <div className="my-10 border-l-2 border-[#A38442] bg-[#FBF8F2] p-6 text-sm leading-relaxed text-[#4A4540] md:p-8">
          ...
          <div className="mt-6 grid grid-cols-2 gap-px overflow-hidden border border-[#DDD5C5] bg-[#DDD5C5] md:grid-cols-5">
            {config.options.map((option) => (
              <div key={option.value} className="bg-[#FBF8F2] p-3 text-center">
                <div className="font-fraunces mb-1 text-lg font-medium text-[#A38442]">{option.value}</div>
        ...
        <form onSubmit={calculateScore}>
          <div>
            {config.items.map((item, index) => (
              <div key={index} className="border-t border-[#EAE3D3] py-8 last:border-b last:border-[#EAE3D3]">
                <span className="font-fraunces mb-3 block text-[13px] tracking-wide text-[#A38442]">
                  ...
                  {config.options.map((option) => (
                    <div key={option.value} className="relative">
                      ...
                      <label
                        ...
                        className="block cursor-pointer select-none border border-[#DDD5C5] bg-[#FBF8F2] px-4 py-3 text-center transition-all duration-150 hover:border-[#A38442] hover:bg-[#F2EBDA] peer-checked:border-[#A38442] peer-checked:bg-[#A38442] peer-checked:text-[#F5F1EA] md:px-2 md:py-3.5"
                      >
                        <span className="font-fraunces mb-0.5 block text-lg font-medium">{option.value}</span>
                        <span className="text-[11px] uppercase tracking-widest text-[#8A8278] peer-checked:text-[#E2D4B5] md:text-[9px]">
                          {option.label}
                        </span>
                      </label>
                    </div>
          ...
          <div className="mt-16 text-center">
            ...
            <button
              type="submit"
              disabled={!isComplete}
              className="bg-[#1A1814] px-12 py-4 text-[14px] font-medium uppercase tracking-[0.06em] text-[#F5F1EA] transition-all duration-200 hover:-translate-y-px hover:bg-[#A38442] disabled:cursor-not-allowed disabled:opacity-40"
            >
              Calculate score
            </button>
          </div>
        ...
        {showResults && results && (
          ...
          >
            ...
            {submitError ? (
              <div className="mb-6 border-l-2 border-[#A38442] bg-[#F2F0EB] p-4 text-sm leading-relaxed text-[#1A1814]">
                {submitError}
              </div>
            ) : null}

            <div className="mb-6 flex items-baseline gap-4">
              <span className="font-fraunces text-7xl font-normal leading-none tracking-tighter text-[#A38442] md:text-8xl">
                {results.total}
              </span>
            ...
            <div className="mb-2 grid grid-cols-5 gap-0.5">
              {[0, 1, 2, 3, 4].map((index) => (
                <div
                  key={index}
                  className={`h-1.5 transition-colors duration-500 ${index <= results.bandIndex ? 'bg-[#A38442]' : 'bg-[#DDD5C5]'
                    }`}
                />
              ))}
            </div>
                ...
                {results.itemScores.map((score, index) => (
                  <div
                    ...
                    className={`relative flex aspect-square items-center justify-center border border-[#DDD5C5] font-fraunces text-lg tabular-nums
                      ...
                      ${score.scored === 1 ? 'bg-[#F2EBDA] text-[#1A1814]' : ''}
                      ${score.scored === 2 ? 'bg-[#E2D4B5] text-[#1A1814]' : ''}
                      ${score.scored === 3 ? 'bg-[#C4AC78] text-white' : ''}
                      ${score.scored === 4 ? 'bg-[#A38442] text-white' : ''}
                    `}
            ...
            <div className="mt-12 flex flex-wrap gap-4">
              ...
              <Link
                ...
                className="text-[12px] bg-[#1A1814] text-[#F5F1EA] px-6 py-3 transition-all hover:bg-[#A38442] flex items-center gap-2"
              >
                View Progress Chart
                <svg
