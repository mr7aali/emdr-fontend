...
const config = {
  name: 'Pain',
  items: [
    { text: 'Physical pain has been part of my days.', reverse: false },
    { text: 'The pain has reached a level that was hard to bear.', reverse: false },
    { text: 'The pain has broken into my sleep.', reverse: false },
    { text: 'The pain has held me back from work or daily tasks.', reverse: false },
    { text: 'The pain has cost me time with people or things I value.', reverse: false },
    { text: 'I have had stretches of being at home in my body, free of pain.', reverse: true },
    { text: 'The pain has dragged my mood down or made me edgy.', reverse: false },
    { text: 'I have given up activities I would normally do because of the pain.', reverse: false },
    { text: 'I have leaned on medication or other interventions to get through.', reverse: false },
    { text: 'The pain has felt unpredictable, slippery, or hard to influence.', reverse: false },
  ],
  bands: [
    {
      max: 9,
      label: 'Minimal',
      description: 'Few signs of significant pain or pain-related impact over the past week.',
    },
    {
      max: 17,
      label: 'Mild',
      description:
        'Some pain present, with mild impact on daily life. Worth tracking but not substantially disrupting day-to-day functioning.',
    },
    {
      max: 25,
      label: 'Moderate',
      description:
        "A meaningful level of pain and pain-related impact over the past week. If you haven't already, please make sure your GP or medical team are aware.",
    },
    {
      max: 32,
      label: 'Marked',
      description:
        'Substantial pain with likely impact on sleep, mood, and activity. Please make sure your GP or medical team are aware of this level of pain - chronic pain often benefits from a combined medical and psychological approach.',
    },
    {
      max: 40,
      label: 'Severe',
      description:
        'A high level of pain with significant impact across multiple areas of life. Please make sure your medical team see this score, and consider whether your current pain management plan needs review.',
    },
  ],
  options: [
    { value: 0, label: 'Never' },
...
export default function Pcl5PtsdChecklistSession7Page() {
  ...
  const calculateScore = async (e) => {
    ...
    try {
      ...
      console.error('Failed to submit pain tracker:', error);
  ...
  return (
    <div
      className={`${fraunces.variable} ${instrumentSans.variable} rounded-2xl bg-[#F5F1EA]/50 font-sans text-[#1A1814] min-h-screen antialiased selection:bg-[#9B5D52]/20`}
    >
      <div className="mx-auto max-w-[720px] px-5 py-10 pb-20 md:px-8 md:py-16 md:pb-32">
        <header className="mb-12 border-b border-[#DDD5C5] pb-8">
          <div className="mb-6 flex items-center gap-3 text-[11px] font-medium uppercase tracking-[0.18em] text-[#8A8278]">
            <span className="h-px w-6 bg-[#9B5D52]"></span>
            InKind EMDR | Symptom Tracker | 07
          </div>
          ...
          <p className="font-fraunces max-w-[560px] text-xl font-light leading-relaxed text-[#4A4540] opsz-36">
            A brief check-in on physical pain and its impact across the past week.
          </p>
        </header>

        <div className="my-10 border-l-2 border-[#9B5D52] bg-[#FBF8F2] p-6 text-sm leading-relaxed text-[#4A4540] md:p-8">
          ...
          <div className="mt-6 grid grid-cols-2 gap-px overflow-hidden border border-[#DDD5C5] bg-[#DDD5C5] md:grid-cols-5">
            {config.options.map((option) => (
              <div key={option.value} className="bg-[#FBF8F2] p-3 text-center">
                <div className="font-fraunces mb-1 text-lg font-medium text-[#9B5D52]">{option.value}</div>
        ...
        <form onSubmit={calculateScore}>
          <div>
            {config.items.map((item, index) => (
              <div key={index} className="border-t border-[#EAE3D3] py-8 last:border-b last:border-[#EAE3D3]">
                <span className="font-fraunces mb-3 block text-[13px] tracking-wide text-[#9B5D52]">
                  ...
                  {config.options.map((option) => (
                    <div key={option.value} className="relative">
                      ...
                      <label
                        ...
                        className="block cursor-pointer select-none border border-[#DDD5C5] bg-[#FBF8F2] px-4 py-3 text-center transition-all duration-150 hover:border-[#9B5D52] hover:bg-[#F1E6E2] peer-checked:border-[#9B5D52] peer-checked:bg-[#9B5D52] peer-checked:text-[#F5F1EA] md:px-2 md:py-3.5"
                      >
                        <span className="font-fraunces mb-0.5 block text-lg font-medium">{option.value}</span>
                        <span className="text-[11px] uppercase tracking-widest text-[#8A8278] peer-checked:text-[#DEC8C2] md:text-[9px]">
                          {option.label}
                        </span>
                      </label>
                    </div>
          ...
          <div className="mt-16 text-center">
            ...
            <button
              type="submit"
              disabled={!isComplete}
              className="bg-[#1A1814] px-12 py-4 text-[14px] font-medium uppercase tracking-[0.06em] text-[#F5F1EA] transition-all duration-200 hover:-translate-y-px hover:bg-[#9B5D52] disabled:cursor-not-allowed disabled:opacity-40"
            >
              Calculate score
            </button>
          </div>
        ...
        {showResults && results && (
          ...
          >
            ...
            {submitError ? (
              <div className="mb-6 border-l-2 border-[#9B5D52] bg-[#F2EDED] p-4 text-sm leading-relaxed text-[#1A1814]">
                {submitError}
              </div>
            ) : null}

            <div className="mb-6 flex items-baseline gap-4">
              <span className="font-fraunces text-7xl font-normal leading-none tracking-tighter text-[#9B5D52] md:text-8xl">
                {results.total}
              </span>
            ...
            <div className="mb-2 grid grid-cols-5 gap-0.5">
              {[0, 1, 2, 3, 4].map((index) => (
                <div
                  key={index}
                  className={`h-1.5 transition-colors duration-500 ${index <= results.bandIndex ? 'bg-[#9B5D52]' : 'bg-[#DDD5C5]'
                    }`}
                />
              ))}
            </div>
                ...
                {results.itemScores.map((score, index) => (
                  <div
                    ...
                    className={`relative flex aspect-square items-center justify-center border border-[#DDD5C5] font-fraunces text-lg tabular-nums
                      ...
                      ${score.scored === 1 ? 'bg-[#F1E6E2] text-[#1A1814]' : ''}
                      ${score.scored === 2 ? 'bg-[#DEC8C2] text-[#1A1814]' : ''}
                      ${score.scored === 3 ? 'bg-[#BF8D80] text-white' : ''}
                      ${score.scored === 4 ? 'bg-[#9B5D52] text-white' : ''}
                    `}
            ...
            <div className="mt-12 flex flex-wrap gap-4">
              ...
              <Link
                ...
                className="text-[12px] bg-[#1A1814] text-[#F5F1EA] px-6 py-3 transition-all hover:bg-[#9B5D52] flex items-center gap-2"
              >
                View Progress Chart
                <svg
